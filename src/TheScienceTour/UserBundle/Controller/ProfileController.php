...
class ProfileController extends ContainerAware {
	...
	public function chatAction($id) {
    	...
    	return $this->container->get('templating')->renderResponse('TheScienceTourUserBundle:Profile:chat.html.twig', array(
    			'user' => $user,
    			'chat' => $chat
    	));
	}
    ...
    public function showAction($tab) {
        ...
        $createdProjects = $projectRepo->findProjectsCreatedBy($user->getId())->execute();
        //$contribProjects = $projectRepo->findProjectsWithContributor($user->getId())->execute();
        	$userskills = $dm->getRepository('TheScienceTourProjectBundle:Skill')->findSkillsWithHelper($user->getId())->execute();
        	$idskills = array(); foreach($userskills as $skill) {$idskills[] = $skill->getId();}
        $contribProjects = $projectRepo->findProjectsWithContributorOrSkills($user->getId(), $idskills)->execute();
             
        $supportedProjects = $projectRepo->findProjectsWithSupporter($user->getId())->execute();
        $followedProjects = $projectRepo->findProjectsWithSubscriber($user->getId())->execute();
        $sponsoredProjects = null;
        if ($this->container->get('security.context')->isGranted('ROLE_RESEARCHER')) {
        	$sponsoredProjects = $projectRepo->findProjectsWithSponsor($user->getId())->execute();
        ...
        if ($this->container->get('security.context')->isGranted('ROLE_SUPER_ANIM')) {
        	$challenges = $dm->getRepository('TheScienceTourChallengeBundle:Challenge')->findAll();
        ...
        return $this->container->get('templating')->renderResponse('FOSUserBundle:Profile:show.html.'.$this->container->getParameter('fos_user.template.engine'), array('user' => $user, 'myChats' => $myChats, 'createdProjects' => $createdProjects, 'contribProjects' => $contribProjects, 'supportedProjects' => $supportedProjects, 'followedProjects' => $followedProjects, 'sponsoredProjects' => $sponsoredProjects, 'drafts' => $drafts, 'createdChallenges' => $createdChallenges, 'otherChallenges' => $otherChallenges, 'tab' => $tab));
    }
	...
	public function editPublicInfoAction() {
		...
		if ($request->getMethod() == "POST") {
			...
			if ($form->isValid()) {
				...
				return new RedirectResponse($this->container->get('router')->generate('fos_user_profile_show', array('tab' => 'mypublicinfo')));
			}
		}
		
		return $this->container->get('templating')->renderResponse('TheScienceTourUserBundle:Profile:editpublicinfo.html.twig',
			array('form' => $form->createView())
		);
	}
	...
	public function publicProfileAction($nickname) {
		$other = $this->container->get('fos_user.user_manager')->findUserByUsername($nickname);
		...
		$dm = $this->container->get('doctrine_mongodb')->getManager();
		$projectRepo = $dm->getRepository('TheScienceTourProjectBundle:Project');
		$projects = $projectRepo->findProjectsWithSponsor($other->getId())->execute();
		...
		return $this->container->get('templating')->renderResponse('TheScienceTourUserBundle:Profile:publicprofile.html.twig', array(
				'other' => $other,
				'projects' => $projects,
	...
	public function addRoleAction() {
		...
		$picRepo = $this->container->get('doctrine_mongodb')->getRepository('TheScienceTourMediaBundle:Media');
		$pictureList = $picRepo->findByContext('researcher_role');
		
		
		$userRole = new UserRole();
		
		$form = $this->container->get('form.factory')->createBuilder('form', $userRole)
		...
		return $this->container->get('templating')->renderResponse('TheScienceTourUserBundle:Profile:adduserrole.html.twig',
