...
class ChallengeController extends Controller {
  ...
  public function addChatMessageAction(Request $request, $id, $idchat) {
    ...
    if ($request->getMethod() == 'POST') {
      ...
      elseif (strlen($mess_nohtml) > 5000) {
        ...
        if ($newChat) {
          ...
          if ($user != $challenge->getCreator()) {
            ...
            $challenge->getCreator()
              ->addNotification("challenge-chats", $challenge->getId());
        ...
        return $this->redirect($this->generateUrl('tst_challenge', array(
          'id'  => $id,
          'tab' => "chats"
        )));
      }
  ...
  public function deleteChatAction($id, $idchat) {
    ...
    if ($user != $challenge->getCreator() && !($this->get('security.context')
        ->isGranted('ROLE_SUPER_ANIM'))
    ) {
      throw new AccessDeniedException();
    }
