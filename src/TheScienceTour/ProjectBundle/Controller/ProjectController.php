...
class ProjectController extends Controller {
	...
	public function projectsAction($filter, $center) {
		if ($form = $this->getRequest()->query->get('form', false)) {
			if ($form['center']) {
	    		return $this->redirect($this->generateUrl('tst_projects', array('filter' => $filter, 'center' => $form['center'])));
			}
		}
    	...
    	$route = array('routeName' => 'tst_projects', 'parameters' => array('filter' => $filter, 'center' => $center));
	...
	public function projectAction($id, $tab) {
    	...
    	$isEditable = ($user == $project->getCreator() || $this->get('security.context')->isGranted('ROLE_PROJECT_MOD'));

    	$geocode = $this->get('the_science_tour_map.map_helper')->getGeocode($project->getPlace());
    	...
    	foreach ($project->getSkills() as $skill) {
    		foreach ($skill->getHelpers() as $helper) {
    			if (array_key_exists($helper->getUsernameCanonical(), $allContribArray)) {
    				...
    				$allContribArray[$helper->getUsernameCanonical()] = array($helper, $skill->getName());
    			}
    		}
    	}
    	foreach ($project->getContributors() as $contrib) {
    		if (!array_key_exists($contrib->getUsernameCanonical(), $allContribArray)) {
    			$allContribArray[$contrib->getUsernameCanonical()] = array($contrib, '');
    		}
    	}
	...
	public function addProjectAction($idchallenge) {
		...
		$form = $this->createFormBuilder($project, array('cascade_validation' => true))
					...
					->add('place', 'places_autocomplete', array(
						...
						'async' => false,
						'language' => 'fr',
						'attr' => array('placeholder' => '', 'oninvalid' => 'javascript:show(0);', 'required' => 'required')
					))
					...
					->add('durationUnit', 'choice', array(
							'choices'   => array('day' => 'Jours', 'week' => 'Semaines', 'month' => "Mois")
					))
					->add('price', 'integer', array('attr' => array('min' => 0), 'required' => false))
					->add('startedAt', 'date', array('empty_value' => '', 'required' => false))
					->add('tools', 'collection', array('type' => new ResourceType(),
													   'allow_add' => true,
													   'allow_delete' => true,
													   'by_reference' => false,
						'required'=>false,
			...
			'required'=> false
					...
					->add('materials', 'collection', array('type' => new ResourceType(),
						  							   'allow_add' => true,
						  							   'allow_delete' => true,
						  							   'by_reference' => false,
						  ))
					->add('premises', 'collection', array('type' => new ResourceType(),
												  		'allow_add' => true,
												  		'allow_delete' => true,
												  		'by_reference' => false,
						  ))
					->add('skills', 'collection', array('type' => new SkillType(),
						  							   'allow_add' => true,
						  							   'allow_delete' => true,
						  							   'by_reference' => false,
						  ))
					->add('draft', 'submit', array('attr' => array('formnovalidate' => 'formnovalidate', 'class' => 'button white_button')))
					->add('publish', 'submit', array('attr' => array('class' => 'button orange_button'), 'validation_groups' => array('Default', 'publish')))
					->getForm();
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) {
				...
				foreach ($project->getMaterials() as $material) {
					if ($material->getCreatorHelpNb() < 0 || $material->getCreatorHelpNb() > $material->getNumber()) {$material->setCreatorHelpNb(0);}
				...
				foreach ($project->getPremises() as $premise) {
					if ($premise->getCreatorHelpNb() < 0 || $premise->getCreatorHelpNb() > $premise->getNumber()) {$premise->setCreatorHelpNb(0);}
	...
	public function editProjectAction($id) {
		...
		if ($user != $project->getCreator() && !($this->get('security.context')->isGranted('ROLE_PROJECT_MOD'))) {
			throw new AccessDeniedException();
		}
		...
		if ($project->getStatus() == 0) {
			$form->add('draft', 'submit', array('attr' => array('formnovalidate' => 'formnovalidate', 'class' => 'button white_button')))
			->add('publish', 'submit', array('attr' => array('class' => 'button orange_button'), 'validation_groups' => array('Default', 'publish')));
		} else {
			$form->add('save', 'submit', array('attr' => array('class' => 'button orange_button'), 'validation_groups' => array('Default', 'publish')));
		}
		$form = $form->getForm();
	...
	public function adminProjectAction($id, $tab) {
		...
		if ($tab == "team") {
			...
			if ($request->getMethod() == 'POST') {
						...
						if ($deleteTools[$i][$j] == "1") {
							...
							if ($helps[$j]->getChat()) {
								foreach ($helps[$j]->getChat()->getUsers() as $chatUser) {
									$chatUser->removeNotification("chat", $helps[$j]->getChat()->getId());
									$dm->persist($chatUser);
								}
							...
							if ($helps[$j]->getNbNeeded() != intval($neededTools[$i][$j])) {
								...
								$helps[$j]->getHelper()->addNotification("chat", $helps[$j]->getChat()->getId());
				...
				return $this->redirect($this->generateUrl('tst_project', array('id' => $project->getId(), 'tab' => "resources")));
			}
	...
	public function adminProjectChatAction($id, $idres, $idhelp) {
		...
		if (!$project->getTools()->contains($res) && !$project->getMaterials()->contains($res) && !$project->getPremises()->contains($res) ) {
		...
		if ($request->getMethod() == 'POST') {
			if ($request->request->has('stop')) {
				...
				$help->getHelper()->removeNotification("chat", $help->getChat()->getId());
				...
				if ($request->request->has('nb')) {
					...
					$help->getHelper()->addNotification("chat", $help->getChat()->getId());
					$dm->persist($help->getHelper());
					$dm->flush();
	...
	public function addNewsAction($id) {
		...
		if (!$project->getTeam()->contains($user) && !($this->get('security.context')->isGranted('ROLE_PROJECT_MOD'))) {
			throw new AccessDeniedException();
		}
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) {
				...
				if ($project->getSubscribers() && $project->getSubscribers()->count()>0) {
				...
				return $this->redirect($this->generateUrl('tst_project', array('id' => $project->getId(), 'tab' => "news")));
			}
		}
	...
	public function editNewsAction($id, $idnews) {
		...
		if ($user != $project->getCreator() && !($this->get('security.context')->isGranted('ROLE_PROJECT_MOD')) && $user != $news->getAuthor()) {
			throw new AccessDeniedException();
		}
	...
	public function resHelpAction($id, $idres) {
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($number > 0 && $res->getActualNumber() + $number <= $res->getNumber()) {
				...
				$this->get('mailer')->send($message);
				$project->getCreator()->addNotification("project-resources", $project->getId());
				$dm->persist($project->getCreator());
				$dm->flush();
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($delegate_username == $user->getUsername()) {
				...
				if (!$delegate) {
					...
					$session->getFlashBag()->add('notice', "Un e-mail vient d'être envoyé à ".$delegate->getUsername().". Vous resterez administrateur de ce projet jusqu'à ce qu'il accepte votre proposition.");

					return $this->redirect($this->generateUrl('tst_project_admin', array('id' => $id, 'tab' => "team")));
				}
			}
		}
	...
	*/
	...
	public function addChatAction($id) {
		...
		$privatizable = $project->getTeam()->contains($user) || $this->get('security.context')->isGranted('ROLE_PROJECT_MOD');
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($mess == null || $mess_nohtml == "") {
				...
				if ($user != $project->getCreator()) {
					...
					$project->getCreator()->addNotification("project-chats", $project->getId());
					$dm->persist($project->getCreator());
				}
				...
				return $this->redirect($this->generateUrl('tst_project', array('id' => $id, 'tab' => "chats")));
			}
	...
	public function addMessageAction($id, $idchat) {
		...
		if ($chat->getPrivate() && !$project->getTeam()->contains($user) && !$this->get('security.context')->isGranted('ROLE_PROJECT_MOD')) {
	...
	public function deleteChatAction($id, $idchat) {
		...
		$dm->persist($project);
		$dm->flush();
		return $this->redirect($this->generateUrl('tst_project', array('id' => $project->getId(), 'tab' => "chats")));
	...
	public function sponsorAction($id) {
		$user = $this->getUser();
		if (!$user || !$this->get('security.context')->isGranted('ROLE_RESEARCHER')) {
			throw new AccessDeniedException();
		}
		$project = $this->get('doctrine_mongodb')
			->getRepository('TheScienceTourProjectBundle:Project')
